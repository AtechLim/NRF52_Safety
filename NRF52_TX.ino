...
void shutdown_gpio() {
  for (uint8_t i = 0; i < 22; i++) {
    if (i != PIN_DRV5032FB && i != LED_RED && i != PIN_WAKEUP_EXT) {
...
void setup() {
  ...
  if (digitalRead(PIN_DRV5032FB) == LOW || digitalRead(PIN_WAKEUP_EXT) == LOW || digitalRead(PAIRING_BUTTON) == LOW) {
    ...
    if (Bluefruit.begin()) {
      ...
      Bluefruit.setTxPower(0); // 0, 4, 8 dBm
      ...
      if (digitalRead(PAIRING_BUTTON) == LOW) {
        // 페어링 모드 광고
        // major=PAIRING_MARKER_MAJOR(65535)로 페어링 신호를 표시하고,
        // minor에 실제 장치 Major 값을 담음.
        beacon = BLEBeacon(beaconUuid, PAIRING_MARKER_MAJOR, BEACON_MAJOR, -59);
        Serial.println("Sending pairing beacon");
      }
      ...
      // 페어링 모드가 아닌 경우 종료 비콘 송출
      if (digitalRead(PAIRING_BUTTON) == LOW) {
        // 페어링 후 일반 광고로 종료? 아니, 그냥 종료.
      } else {
...
/**
 * Arduino loop 함수
 * 시스템이 슬립 모드로 진입하므로 실제로는 실행되지 않음
 */
void loop() {
  // 시스템은 setup()에서 SYSTEMOFF 모드로 진입하므로
  // 이 함수는 실행되지 않습니다.
}